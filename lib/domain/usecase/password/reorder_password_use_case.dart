import 'package:flutter_password_saver/common/base/base_use_case.dart';
import 'package:flutter_password_saver/domain/model/password.dart';
import 'package:flutter_password_saver/domain/repository/password_repo.dart';
import 'package:injectable/injectable.dart';

@injectable
class ReOrderPasswordUseCase implements BaseUseCase<List<Password>, bool> {
  ReOrderPasswordUseCase(this._passwordRepository);

  final PasswordRepository _passwordRepository;

  @override
  Future<bool> execute(List<Password> input) {
    return _passwordRepository.reOrderPasswords(input);
  }
}
